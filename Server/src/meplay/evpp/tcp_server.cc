...
TCPServer::TCPServer(EventLoop* loop,
    ...
    , next_conn_id_(0) {
    MPLOG_TRACE << "name=" << name << " listening addr " << laddr << " thread_num=" << thread_num;
...
void TCPServer::StopInLoop(DoneCallback on_stopped_cb) {
    ...
    if (connections_.empty()) {
        ...
        for (auto& c : connections_) {
            if (c.second->IsConnected()) {
                MPLOG_TRACE << "close connection id=" << c.second->id() << " fd=" << c.second->fd();
                c.second->Close();
            } else {
                MPLOG_TRACE << "Do not need to call Close for this TCPConn it may be doing disconnecting. TCPConn=" << c.second.get() << " fd=" << c.second->fd() << " status=" << StatusToString();
            }
        }
    ...
    MPLOG_TRACE << "exited, status=" << StatusToString();
}

void TCPServer::StopThreadPool() {
    MPLOG_TRACE << "pool=" << tpool_.get();
    assert(loop_->IsInLoopThread());
    assert(IsStopping());
...
void TCPServer::HandleNewConn(evpp_socket_t sockfd,
    ...
    MPLOG_TRACE << "fd=" << sockfd;
    assert(loop_->IsInLoopThread());
    if (IsStopping()) {
        MPLOG_INFO << "this=" << this << " The server is at stopping status. Discard this socket fd=" << sockfd << " remote_addr=" << remote_addr;
...
void TCPServer::RemoveConnection(const TCPConnPtr& conn) {
    MPLOG_TRACE << "conn=" << conn.get() << " fd="<< conn->fd() << " connections_.size()=" << connections_.size();
    auto f = [this, conn]() {
