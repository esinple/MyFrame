...
void Service::Stop() {
    MPLOG_TRACE << "http service is stopping";
    assert(listen_loop_->IsInLoopThread());
    ...
    MPLOG_TRACE << "http service stopped";
}


void Service::Pause() {
    assert(listen_loop_->IsInLoopThread());
    MPLOG_TRACE << "http service pause";
#if LIBEVENT_VERSION_NUMBER >= 0x02001500
    if (evhttp_bound_socket_) {
        evconnlistener_disable(evhttp_bound_socket_get_listener(evhttp_bound_socket_));
    }
#else
    MPLOG_ERROR << "Not support!".;
...
void Service::Continue() {
    assert(listen_loop_->IsInLoopThread());
    MPLOG_TRACE << "http service continue";
#if LIBEVENT_VERSION_NUMBER >= 0x02001500
    if (evhttp_bound_socket_) {
...
void Service::HandleRequest(struct evhttp_request* req) {
    ...
    assert(listen_loop_->IsInLoopThread());
    MPLOG_TRACE << "handle request " << req << " url=" << req->uri;
...
void Service::DefaultHandleRequest(const ContextPtr& ctx) {
    MPLOG_TRACE << "url=" << ctx->original_uri();
...
struct Response {
    ...
    ~Response() {
        ...
        // MPLOG_TRACE << "free request " << req->uri;
    }
...
void Service::SendReply(const ContextPtr& ctx, const std::string& response_data) {
    ...
    MPLOG_TRACE << "send reply in working thread";
    ...
    auto f = [this, response]() {
        // In the main HTTP listening thread
        assert(listen_loop_->IsInLoopThread());
        MPLOG_TRACE << "send reply in listening thread. evhttp_=" << evhttp_;

        auto x = response->ctx.get();

        // At this moment, this Service maybe already stopped.
        if (!evhttp_) {
            MPLOG_INFO << "this=" << this << " Service has been stopped.";
            return;
        }
    ...
    if (listen_loop_->IsRunning()) {
        MPLOG_TRACE << "dispatch this SendReply to listening thread";
        listen_loop_->RunInLoop(f);
    } else {
        MPLOG_INFO << "this=" << this << " listening thread is going to stop. we discards this request.";
