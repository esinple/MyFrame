...
void TCPClient::Connect() {
    MPLOG_INFO << "remote_addr=" << remote_addr();
    auto f = [this]() {
...
void TCPClient::DisconnectInLoop() {
	MPLOG_INFO << "TCPClient::DisconnectInLoop this=" << this << " remote_addr=" << remote_addr_;
	assert(loop_->IsInLoopThread());
	auto_reconnect_.store(false);

	if (conn_) {
		MPLOG_TRACE << "Close the TCPConn " << conn_.get() << " status=" << conn_->StatusToString();
	...
	if (connector_)
	{
		if (connector_->IsConnected() || connector_->IsDisconnected()) {
			MPLOG_TRACE << "Nothing to do with connector_, Connector::status=" << connector_->status();
		}
		else {
...
void TCPClient::Reconnect() {
    MPLOG_TRACE << "Try to reconnect to " << remote_addr_ << " in " << reconnect_interval_.Seconds() << "s again";
    Connect();
}
...
void TCPClient::OnConnection(evpp_socket_t sockfd, const std::string& laddr) {
    if (sockfd < 0) {
        MPLOG_TRACE << "Failed to connect to " << remote_addr_ << ". errno=" << errno << " " << strerror(errno);
    ...
    MPLOG_TRACE << "Successfully connected to " << remote_addr_;
    assert(loop_->IsInLoopThread());
