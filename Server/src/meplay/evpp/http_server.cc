...
bool Server::Init(int listen_port) {
    ...
    if (!lt.hservice->Listen(listen_port)) {
        int serrno = errno;
        MPLOG_ERROR << "this=" << this << " http server listen at port " << listen_port << " failed. errno=" << serrno << " " << strerror(serrno);
        lt.hservice->Stop();
        return false;
    }
...
bool Server::Init(const std::string& listen_ports/*"80,8080,443"*/) {
    ...
    for (auto& s : vec) {
        ...
        if (i <= 0) {
            MPLOG_ERROR << "this=" << this << " Cannot convert [" << s << "] to a integer. 'listen_ports' format wrong.";
            return false;
        }
        v.push_back(i);
    }
...
bool Server::Start() {
    ...
    if (!rc) {
        MPLOG_ERROR << "this=" << this << " start thread pool failed.";
        return false;
    }
    ...
    for (auto& lt : listen_threads_) {
        ...
        auto http_close_fn = [hservice, this]() {
            hservice->Stop();
            MPLOG_TRACE << "http service at 0.0.0.0:" << hservice->port() << " has stopped.";
        ...
        rc = lthread->Start(true,
                            ...
                            http_close_fn);
        if (!rc) {
            MPLOG_ERROR << "this=" << this << " start listening thread failed.";
            return false;
        }
    ...
    MPLOG_TRACE << "http server is running" ;
...
void Server::Stop() {
    assert(IsRunning());
    MPLOG_TRACE << "http server is stopping";
    ...
    MPLOG_TRACE << "http server stopped";
}

void Server::Pause() {
    MPLOG_TRACE << "http server pause";
    for (auto& lt : listen_threads_) {
...
void Server::Continue() {
    MPLOG_TRACE << "http server continue";
    for (auto& lt : listen_threads_) {
        EventLoop* loop = lt.thread->loop();
        std::shared_ptr<Service>& hs = lt.hservice;
        auto f = [hs]() {
...
void Server::Dispatch(EventLoop* listening_loop,
    ...
    MPLOG_TRACE << "dispatch request " << ctx->req() << " url=" << ctx->original_uri() << " in main thread. status=" << StatusToString();
    if (!IsRunning()) {
        MPLOG_INFO << "The listening thread is not running, may be it is stopping now.";
    ...
    auto f = [loop, ctx, response_callback, user_callback, this]() {
        MPLOG_TRACE << "process request " << ctx->req()
...
EventLoop* Server::GetNextLoop(EventLoop* default_loop, const ContextPtr& ctx) {
    ...
    if (sa) {
        ...
        MPLOG_INFO << "http remote address " << sock::ToIPPort(r);
