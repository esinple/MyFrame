...
DNSResolver::DNSResolver(EventLoop* evloop, const std::string& h, Duration timeout, const Functor& f)
    ...
    MPLOG_TRACE << "tid=" << std::this_thread::get_id() << " this=" << this;
}
...
void DNSResolver::SyncDNSResolve() {
    ...
    if (err != 0) {
        MPLOG_ERROR << "this=" << this << " getaddrinfo failed. err=" << err << " " << gai_strerror(err);
    } else {
        for (struct addrinfo* rp = answer; rp != nullptr; rp = rp->ai_next) {
            ...
            MPLOG_TRACE << "host=" << host_ << " resolved a ip=" << inet_ntoa(a->sin_addr);
        }
    }
...
void DNSResolver::AsyncDNSResolve() {
    ...
    MPLOG_TRACE << "call shared_from_this";
...
void DNSResolver::OnResolved(int errcode, struct addrinfo* addr) {
    if (errcode != 0) {
        if (errcode != EVUTIL_EAI_CANCEL) {
            ClearTimer();
            MPLOG_ERROR << "DNS resolve failed, "
                << ", error code: " << errcode
                << ", error msg: " << evutil_gai_strerror(errcode);
        } else {
            MPLOG_INFO << "DNS resolve cancel, may be timeout";
        }

        MPLOG_INFO << "delete DNS base. errcode=" << errcode << " " << strerror(errcode);
    ...
    if (addr == nullptr) {
        MPLOG_ERROR << "this=" << this << " dns resolve error, addr can not be nullptr";

        MPLOG_TRACE << "delete dns base";
        evdns_base_free(dnsbase_, 0);
        dnsbase_ = nullptr;
        ClearTimer();
        OnResolved();
    ...
    if (addr->ai_canonname) {
        MPLOG_TRACE << "resolve canon name: " << addr->ai_canonname;
    }
...
void DNSResolver::OnResolved(int errcode, struct addrinfo* addr, void* arg) {
    ...
    MPLOG_TRACE << "this->use_count=" << pp->use_count();
