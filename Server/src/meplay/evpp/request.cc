...
failed:
    ...
    if (retried_ < retry_number_) {
        MPLOG_INFO << "this=" << this << " http request failed : " << errmsg << " retried=" << retried_ << " max retry_time=" << retry_number_ << ". Try again.";
        Retry();
        return;
    }
...
void Request::HandleResponse(struct evhttp_request* r) {
    ...
    if (r) {
        if (r->response_code == HTTP_OK || retried_ >= retry_number_) {
            MPLOG_INFO << "this=" << this << " response_code=" << r->response_code << " retried=" << retried_ << " max retry_time=" << retry_number_;
    ...
    // Retry
    if (retried_ < retry_number_) {
        MPLOG_INFO << "this=" << this << " response_code=" << (r ? r->response_code : 0) << " retried=" << retried_ << " max retry_time=" << retry_number_ << ". Try again";
        Retry();
        return;
    }
