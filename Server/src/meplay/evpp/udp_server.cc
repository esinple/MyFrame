    ...
    ~RecvThread() {
        ...
        if (this->thread_ && this->thread_->joinable()) {
            try {
                ...
                MPLOG_ERROR << "Caught a system_error:" << e.what();
            }
        }
    }
...
bool Server::Init(const std::string& listen_ports/*like "53,5353,1053"*/) {
    ...
    for (auto& s : vec) {
        ...
        if (i <= 0) {
            MPLOG_ERROR << "Cannot convert [" << s << "] to a integer. 'listen_ports' format wrong.";
            return false;
        }
        v.push_back(i);
    }
...
bool Server::Start() {
    if (!message_handler_) {
        MPLOG_ERROR << "MessageHandler DO NOT set!";
        return false;
    }
...
void Server::RecvingLoop(RecvThread* thread) {
    MPLOG_INFO << "UDPServer is running at 0.0.0.0:" << thread->port();
    ...
    while (true) {
        ...
        int readn = ::recvfrom(thread->fd(), (char*)recv_msg->WriteBegin(), static_cast<int>(recv_buf_size_), 0, recv_msg->mutable_remote_addr(), &addr_len);
        if (readn >= 0) {
            MPLOG_TRACE << "fd=" << thread->fd() << " port=" << thread->port()
            ...
            MPLOG_ERROR << "errno=" << eno << " " << strerror(eno);
        }
    }

    MPLOG_INFO << "fd=" << thread->fd() << " port=" << thread->port() << " UDP server existed.";
