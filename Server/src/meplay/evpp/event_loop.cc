...
EventLoop::EventLoop(struct event_base* base)
    ...
    assert(rc);
    if (!rc) {
        MPLOG_FATAL << "PipeEventWatcher init failed.";
    }
...
void EventLoop::Run() {
    ...
    if (rc == 1) {
        MPLOG_ERROR << "event_base_dispatch error: no event registered";
    } else if (rc == -1) {
        int serrno = errno;
        MPLOG_ERROR << "event_base_dispatch error " << serrno << " " << strerror(serrno);
    }

    // Make sure watcher_ does construct, initialize and destruct in the same thread.
    watcher_.reset();
    MPLOG_TRACE << "EventLoop stopped, tid=" << std::this_thread::get_id();
...
void EventLoop::Stop() {
    ...
    MPLOG_TRACE << "EventLoop::Stop";
    QueueInLoop(std::bind(&EventLoop::StopInLoop, this));
}

void EventLoop::StopInLoop() {
    MPLOG_TRACE << "EventLoop is stopping now, tid=" << std::this_thread::get_id();
    assert(status_.load() == kStopping);

    auto f = [this]() {
        for (int i = 0;;i++) {
            MPLOG_TRACE << "calling DoPendingFunctors index=" << i;
            DoPendingFunctors();
    ...
    MPLOG_TRACE << "before event_base_loopexit, we invoke DoPendingFunctors";

    f();

    MPLOG_TRACE << "start event_base_loopexit";
    event_base_loopexit(evbase_, nullptr);
    MPLOG_TRACE << "after event_base_loopexit, we invoke DoPendingFunctors";

    f();

    MPLOG_TRACE << "end of StopInLoop";
}
...
void EventLoop::AfterFork() {
    ...
    if (rc != 0) {
        MPLOG_FATAL << "event_reinit failed!";
        abort();
    }
...
void EventLoop::QueueInLoop(const Functor& cb) {
    MPLOG_TRACE << "pending_functor_count_=" << pending_functor_count_ << " PendingQueueSize=" << GetPendingQueueSize() << " notified_=" << notified_.load();
    {
#ifdef H_HAVE_BOOST
    ...
    MPLOG_TRACE << "queued a new Functor. pending_functor_count_=" << pending_functor_count_ << " PendingQueueSize=" << GetPendingQueueSize() << " notified_=" << notified_.load();
    if (!notified_.load()) {
        MPLOG_TRACE << "call watcher_->Nofity() notified_.store(true)";
        ...
        if (watcher_) {
            ...
            MPLOG_TRACE << "status=" << StatusToString();
            assert(!IsRunning());
        }
    } else {
         MPLOG_TRACE << "No need to call watcher_->Nofity()";
    }
}
...
void EventLoop::QueueInLoop(Functor&& cb) {
    ...
    if (!notified_.load()) {
        ...
        if (watcher_) {
            ...
            MPLOG_TRACE << "watcher_ is empty, maybe we call EventLoop::QueueInLoop on a stopped EventLoop. status=" << StatusToString();
            assert(!IsRunning());
        }
    } else {
        MPLOG_TRACE << "No need to call watcher_->Nofity()";
