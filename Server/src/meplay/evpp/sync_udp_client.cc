...
bool Client::Connect() {
    ...
    if (ret != 0) {
        MPLOG_ERROR << "Failed to connect to remote "
...
std::string Client::DoRequest(const std::string& data, uint32_t timeout_ms) {
    if (!Send(data)) {
        int eno = errno;
        MPLOG_ERROR << "sent failed, errno=" << eno << " " << strerror(eno) << " , dlen=" << data.size();
        return "";
    }
    ...
    if (readn >= 0) {
        ...
        MPLOG_ERROR << "errno=" << err << " " << strerror(err) << " recvfrom return -1";
    }
