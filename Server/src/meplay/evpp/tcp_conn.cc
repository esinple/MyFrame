...
TCPConn::TCPConn(EventLoop* l,
    ...
    MPLOG_TRACE << "TCPConn::[" << name_ << "] channel=" << chan_.get() << " fd=" << sockfd << " addr=" << AddrToString();
}

TCPConn::~TCPConn() {
    MPLOG_TRACE << "name=" << name()
        << " channel=" << chan_.get()
...
void TCPConn::Close() {
    MPLOG_TRACE << "fd=" << fd_ << " status=" << StatusToString() << " addr=" << AddrToString();
...
void TCPConn::SendInLoop(const void* data, size_t len) {
    ...
    if (status_ == kDisconnected) {
        MPLOG_INFO << "disconnected, give up writing";
        return;
    }
    ...
    if (!chan_->IsWritable() && output_buffer_.length() == 0) {
        ...
        if (nwritten >= 0) {
            ...
            if (!EVUTIL_ERR_RW_RETRIABLE(serrno)) {
                MPLOG_ERROR << "SendInLoop write failed errno=" << serrno << " " << strerror(serrno);
...
void TCPConn::HandleRead() {
    ...
    if (n > 0) {
        ...
        if (type() == kOutgoing) {
            ...
            MPLOG_TRACE << "fd=" << fd_ << ". We read 0 bytes and close the socket.";
            status_ = kDisconnecting;
            HandleClose();
        } else {
            ...
            if (close_delay_.IsZero()) {
                MPLOG_TRACE << "channel (fd=" << chan_->fd() << ") DisableReadEvent. delay time " << close_delay_.Seconds() << "s. We close this connection immediately";
                DelayClose();
            } else {
                ...
                MPLOG_TRACE << "channel (fd=" << chan_->fd() << ") DisableReadEvent. And set a timer to delay close this TCPConn, delay time " << close_delay_.Seconds() << "s";
        ...
        if (EVUTIL_ERR_RW_RETRIABLE(serrno)) {
            MPLOG_TRACE << "errno=" << serrno << " " << strerror(serrno);
        } else {
            MPLOG_TRACE << "errno=" << serrno << " " << strerror(serrno) << " We are closing this connection now.";
            HandleError();
        }
    }
}
...
void TCPConn::HandleWrite() {
    ...
    if (n > 0) {
        ...
        if (EVUTIL_ERR_RW_RETRIABLE(serrno)) {
            MPLOG_INFO << "this=" << this << " TCPConn::HandleWrite errno=" << serrno << " " << strerror(serrno);
        } else {
            HandleError();
        }
    }
...
void TCPConn::DelayClose() {
    assert(loop_->IsInLoopThread());
    MPLOG_TRACE << "addr=" << AddrToString() << " fd=" << fd_ << " status_=" << StatusToString();
    status_ = kDisconnecting;
...
void TCPConn::HandleClose() {
    ...
    if (delay_close_timer_) {
        MPLOG_TRACE << "loop=" << loop_ << " Cancel the delay closing timer.";
    ...
    MPLOG_TRACE << "addr=" << AddrToString() << " fd=" << fd_ << " status_=" << StatusToString() << " use_count=" << conn.use_count();
    status_ = kDisconnected;
}

void TCPConn::HandleError() {
    MPLOG_TRACE << "fd=" << fd_ << " status=" << StatusToString();
    status_ = kDisconnecting;
    HandleClose();
}
