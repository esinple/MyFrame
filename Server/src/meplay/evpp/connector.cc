...
Connector::Connector(EventLoop* l, TCPClient* client)
    ...
    MPLOG_TRACE << "raddr=" << remote_addr_;
...
void Connector::Start() {
    MPLOG_TRACE << "Try to connect " << remote_addr_ << " status=" << StatusToString();
    assert(loop_->IsInLoopThread());
    ...
    MPLOG_TRACE << "The remote address " << remote_addr_ << " is a host, try to resolve its IP address.";
...
void Connector::Cancel() {
    MPLOG_TRACE << "Cancel to connect " << remote_addr_ << " status=" << StatusToString();
    assert(loop_->IsInLoopThread());
    if (dns_resolver_) {
...
void Connector::Connect() {
    MPLOG_TRACE << remote_addr_ << " status=" << StatusToString();
    ...
    if (!laddr.empty()) {
        ...
        if (rc != 0) {
            int serrno = errno;
            MPLOG_ERROR << "bind failed, errno=" << serrno << " " << strerror(serrno);
            HandleError();
            return;
        }
    }
    ...
    MPLOG_TRACE << "new FdChannel p=" << chan_.get() << " fd=" << chan_->fd();
...
void Connector::HandleWrite() {
    ...
    if (status_ == kDisconnected) {
        ...
        MPLOG_INFO << "fd=" << chan_->fd() << " remote_addr=" << remote_addr_ << " receive write event when socket is closed";
        return;
    }
    ...
    if (getsockopt(chan_->fd(), SOL_SOCKET, SO_ERROR, (char*)&err, (socklen_t*)&len) != 0) {
        err = errno;
        MPLOG_ERROR << "getsockopt failed err=" << err << " " << strerror(err);
    }
...
void Connector::HandleError() {
    ...
    MPLOG_ERROR << "this=" << this << " status=" << StatusToString() << " fd=" << fd_  << " use_count=" << self.use_count() << " errno=" << serrno << " " << strerror(serrno);
    ...
    if (owner_tcp_client_->auto_reconnect()) {
        ...
        if (fd_ > 0) {
            MPLOG_TRACE << "Connector::HandleError close(" << fd_ << ")";
            assert(own_fd_);
            EVUTIL_CLOSESOCKET(fd_);
            fd_ = INVALID_SOCKET;
        }

        MPLOG_TRACE << "loop=" << loop_ << " auto reconnect in " << owner_tcp_client_->reconnect_interval().Seconds() << "s thread=" << std::this_thread::get_id();
...
void Connector::OnConnectTimeout() {
    MPLOG_INFO << "this=" << this << " Connector::OnConnectTimeout status=" << StatusToString() << " fd=" << fd_ << " this=" << this;
...
void Connector::OnDNSResolved(const std::vector <struct in_addr>& addrs) {
    MPLOG_TRACE << "addrs.size=" << addrs.size() << " this=" << this;
    if (addrs.empty()) {
        MPLOG_ERROR << "this=" << this << " DNS Resolve failed. host=" << dns_resolver_->host();
        HandleError();
        return;
    }
