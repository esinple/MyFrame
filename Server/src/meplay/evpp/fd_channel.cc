...
void FdChannel::AttachToLoop() {
    ...
    if (EventAdd(event_, nullptr) == 0) {
        MPLOG_TRACE << "fd=" << fd_ << " watching event " << EventsToString();
        attached_ = true;
    } else {
        MPLOG_ERROR << "this=" << this << " fd=" << fd_ << " with event " << EventsToString() << " attach to event loop failed";
    }
}
...
void FdChannel::DetachFromLoop() {
    ...
    if (EventDel(event_) == 0) {
        attached_ = false;
        MPLOG_TRACE << "fd=" << fd_ << " detach from event loop";
    } else {
        MPLOG_ERROR << "DetachFromLoop this=" << this << "fd=" << fd_ << " with event " << EventsToString() << " detach from event loop failed";
    }
}
...
void FdChannel::HandleEvent(evpp_socket_t sockfd, short which) {
    assert(sockfd == fd_);
    MPLOG_TRACE << "fd=" << sockfd << " " << EventsToString();
