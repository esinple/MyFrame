...
namespace sock {
evpp_socket_t CreateNonblockingSocket(bool reuse) {
    int serrno = 0;
    ...
    if (fd == -1) {
        serrno = errno;
        MPLOG_ERROR << "socket error " << strerror(serrno);
    ...
    if (fcntl(fd, F_SETFD, 1) == -1) {
        serrno = errno;
        MPLOG_FATAL << "fcntl(F_SETFD)" << strerror(serrno);
        goto out;
    }
#endif
...
evpp_socket_t CreateUDPServer(int port) {
    ...
    if (::bind(fd, (struct sockaddr*)&local, sizeof(struct sockaddr))) {
        int serrno = errno;
        MPLOG_ERROR << "socket bind error=" << serrno << " " << strerror(serrno);
        return INVALID_SOCKET;
    }
...
bool ParseFromIPPort(const char* address, struct sockaddr_storage& ss) {
    ...
    if (rc == 0) {
        MPLOG_INFO << "ParseFromIPPort inet_pton(AF_INET '" << host.data() << "', ...) rc=0. " << host.data() << " is not a valid IP address. Maybe it is a hostname.";
        return false;
    } else if (rc < 0) {
        int serrno = errno;
        if (serrno == 0) {
            MPLOG_INFO << "[" << host.data() << "] is not a IP address. Maybe it is a hostname.";
        } else {
            MPLOG_INFO << "ParseFromIPPort inet_pton(AF_INET, '" << host.data() << "', ...) failed : " << strerror(serrno);
        }
        return false;
    }
...
bool SplitHostPort(const char* address, std::string& host, int& port) {
    ...
    if (index == std::string::npos) {
        MPLOG_ERROR << "Address specified error <" << address << ">. Cannot find ':'";
        return false;
    }
    ...
    if (host[0] == '[') {
        if (*host.rbegin() != ']') {
            MPLOG_ERROR << "Address specified error <" << address << ">. '[' ']' is not pair.";
            return false;
        }
...
struct sockaddr_storage GetLocalAddr(evpp_socket_t sockfd) {
    ...
    if (::getsockname(sockfd, sockaddr_cast(&laddr), &addrlen) < 0) {
        MPLOG_ERROR << "GetLocalAddr:" << strerror(errno);
        memset(&laddr, 0, sizeof laddr);
    }
...
std::string ToIPPort(const struct sockaddr_storage* ss) {
    ...
    if (ss->ss_family == AF_INET) {
        ...
        MPLOG_ERROR << "unknown socket family connected";
        return empty_string;
    }
...
void SetTimeout(evpp_socket_t fd, uint32_t timeout_ms) {
    ...
    if (ret != 0) {
        int err = errno;
        MPLOG_ERROR << "setsockopt SO_RCVTIMEO ERROR " << err << strerror(err);
    }
}
...
void SetKeepAlive(evpp_socket_t fd, bool on) {
    ...
    if (rc != 0) {
        int serrno = errno;
        MPLOG_ERROR << "setsockopt(SO_KEEPALIVE) failed, errno=" << serrno << " " << strerror(serrno);
    }
}
...
void SetReuseAddr(evpp_socket_t fd) {
    ...
    if (rc != 0) {
        int serrno = errno;
        MPLOG_ERROR << "setsockopt(SO_REUSEADDR) failed, errno=" << serrno << " " << strerror(serrno);
    }
}
...
void SetReusePort(evpp_socket_t fd) {
    ...
    int rc = ::setsockopt(fd, SOL_SOCKET, SO_REUSEPORT,
                          reinterpret_cast<const char*>(&optval), static_cast<socklen_t>(sizeof optval));
    MPLOG_INFO << "setsockopt SO_REUSEPORT fd=" << fd << " rc=" << rc;
    if (rc != 0) {
        int serrno = errno;
        MPLOG_ERROR << "setsockopt(SO_REUSEPORT) failed, errno=" << serrno << " " << strerror(serrno);
    }
#endif
}
...
void SetTCPNoDelay(evpp_socket_t fd, bool on) {
    ...
    if (rc != 0) {
        int serrno = errno;
        MPLOG_ERROR << "setsockopt(TCP_NODELAY) failed, errno=" << serrno << " " << strerror(serrno);
    }
}
