...
bool EventWatcher::Watch(Duration timeout) {
    ...
    if (attached_) {
        ...
        if (EventDel(event_) != 0) {
            MPLOG_ERROR << "event_del failed. fd=" << this->event_->ev_fd << " event_=" << event_;
    ...
    if (EventAdd(event_, timeoutval) != 0) {
        MPLOG_ERROR << "event_add failed. fd=" << this->event_->ev_fd << " event_=" << event_;
        return false;
    }
    attached_ = true;
    return true;
...
bool PipeEventWatcher::DoInit() {
    ...
    if (evutil_socketpair(AF_UNIX, SOCK_STREAM, 0, pipe_) < 0) {
        int err = errno;
        MPLOG_ERROR << "create socketpair ERROR errno=" << err << " " << strerror(err);
        goto failed;
    }
...
void PipeEventWatcher::HandlerFn(evpp_socket_t fd, short /*which*/, void* v) {
    MPLOG_INFO << "PipeEventWatcher::HandlerFn fd=" << fd << " v=" << v;
