...
void InvokeTimer::Start() {
    MPLOG_TRACE << "loop=" << loop_ << " refcount=" << self_.use_count();
    auto f = [this]() {
        ...
        timer_->Init();
        timer_->AsyncWait();
        MPLOG_TRACE << "timer=" << timer_.get() << " loop=" << loop_ << " refcount=" << self_.use_count() << " periodic=" << periodic_ << " timeout(ms)=" << timeout_.Milliseconds();
    };
...
void InvokeTimer::OnTimerTriggered() {
    MPLOG_TRACE << "loop=" << loop_ << " use_count=" << self_.use_count();
    functor_();
