...
#if (!defined(ELPP_DISABLE_SYSTEM_LOGS) && (ELPP_LOGGING_ENABLED))
#   define ELPP_SYSTEM_LOG 1
#else
#   define ELPP_SYSTEM_LOG 0
#endif  // (!defined(ELPP_DISABLE_TRACE_LOGS) && (ELPP_LOGGING_ENABLED))
...
#   define ELPP_COUT_LINE(logLine) logLine << std::flush
	...
	public:
		...
		static const base::type::EnumType kMaxValid = static_cast<base::type::EnumType>(Level::System);
		...
		static const char* convertToString(Level level) {
			...
			if (level == Level::System) return "SYSTEM";
			return "UNKNOWN";
		}
		...
		static Level convertFromString(const char* levelStr) {
			...
			if ((strcmp(levelStr, "TRACE") == 0) || (strcmp(levelStr, "trace") == 0))
				return Level::Trace;
			if ((strcmp(levelStr, "SYSTEM") == 0) || (strcmp(levelStr, "system") == 0))
				return Level::System;
	...
	enum class ConfigurationType : base::type::EnumType {
		...
		/// @brief Specified Time format for rolling log file
	...
	public:
		...
		static ConfigurationType convertFromString(const char* configStr) {
			...
			if ((strcmp(configStr, "LOG_FILE_ROLLING_TIME") == 0) || (strcmp(configStr, "log_file_rolling_time") == 0))
	...
	namespace base {
		...
		namespace consts {
			...
			static const base::type::char_t* kSystemLevelLogValue = ELPP_LITERAL("SYSTEM");
			...
			static const base::type::char_t* kSystemLevelShortLogValue = ELPP_LITERAL("SYS");
...
#   endif  // ELPP_OS_UNIX
	...
	typedef std::function<void(const char*, std::size_t, std::tuple<el::base::RollingLogFileBasis,struct tm,std::string>)> PreRollOutCallback;
	namespace base {
		static inline void defaultPreRollOutCallback(const char*, std::size_t, std::tuple<el::base::RollingLogFileBasis, struct tm,std::string>) {}
		...
		namespace utils {
			...
			public:
				...
				static inline std::string& ltrim(std::string& str) {
					...
					str.erase(str.begin(), std::find_if(str.begin(), str.end(), [](const char c)->bool {return std::isspace(c) == 0 ; }));
					return str;
				}
				...
				static inline std::string& rtrim(std::string& str) {
					str.erase(std::find_if(str.rbegin(), str.rend(), [](const char c)->bool {return std::isspace(c) == 0; }).base(), str.end());
					return str;
				}
...
#   endif  // ELPP_COMPILER_MSVC
	...
	namespace base {
		...
		protected:
			...
			virtual void updateFormatSpec(void) ELPP_FINAL {
				...
				else if (m_level == Level::System) {
					base::utils::Str::replaceFirstWithEscape(m_format, base::consts::kSeverityLevelFormatSpecifier,
						base::consts::kSystemLevelLogValue);
					base::utils::Str::replaceFirstWithEscape(m_format, base::consts::kSeverityLevelShortFormatSpecifier,
						base::consts::kSystemLevelShortLogValue);
				}
	...
	public:
		...
		void setToDefault(void) {
			...
			set(Level::System, ConfigurationType::Format, std::string("%datetime %level [%logger] [%func] [%loc] %msg"));
		}
		...
		void setRemainingToDefault(void) {
			...
			unsafeSetIfNotExist(Level::System, ConfigurationType::Format, std::string("%datetime %level [%logger] [%func] [%loc] %msg"));
		}
	...
	namespace base {
		...
		private:
			...
			struct::tm m_fileLastTimeMap; // add to record last time, for changing file name
			...
			bool CompareDateTimeForRollingLog(Level level, std::string& logFileRollingTime) {
				struct::tm currentTime = base::utils::DateTime::getCurrentTm();
				struct::tm modifyTime = m_fileLastTimeMap;
			...
			void insertFile(Level level, const std::string& fullFilename) {
				...
				//m_fileLastTimeMap = base::utils::DateTime::getCurrentTm();
			}
			...
			bool unsafeValidateFileRolling(Level level, const PreRollOutCallback& PreRollOutCallback) {
				...
				if (maxLogFileSize != 0 && currFileSize >= (int64_t)maxLogFileSize) {
					...
					PreRollOutCallback(fname.c_str(), currFileSize, 
						std::make_tuple(el::base::RollingLogFileBasis::RollLog_FileSize,m_fileLastTimeMap,logFileRollingTime));
					fs->open(fname, std::fstream::out | std::fstream::trunc);
					m_fileLastTimeMap = base::utils::DateTime::getCurrentTm();
					return true;
				}
				if(!CompareDateTimeForRollingLog(level, logFileRollingTime))
				{
					ELPP_INTERNAL_INFO(1, "Truncating log file [" << fname << "] as a result of configurations for level ["
						<< LevelHelper::convertToString(level) << "]");
					fs->close();
...
#   define LOGGER_LEVEL_WRITERS_SIGNATURES(FUNCTION_NAME)\
    ...
    inline void FUNCTION_NAME(const T&);
		...
		LOGGER_LEVEL_WRITERS_SIGNATURES(info)
			...
			LOGGER_LEVEL_WRITERS_SIGNATURES(system)
...
#   if ELPP_SYSTEM_LOG
		LOGGER_LEVEL_WRITERS(system, Level::System)
#   else
		LOGGER_LEVEL_WRITERS_DISABLED(system, Level::System)
#   endif // ELPP_SYSTEM_LOG
...
#endif  // ELPP_TRACE_LOG
#if ELPP_SYSTEM_LOG
#   define CSYSTEM(writer, dispatchAction, ...) ELPP_WRITE_LOG(writer, el::Level::System, dispatchAction, __VA_ARGS__)
#else
#   define CSYSTEM(writer, dispatchAction, ...) el::base::NullWriter()
#endif  // ELPP_TRACE_LOG
#if ELPP_VERBOSE_LOG
...
#endif  // ELPP_TRACE_LOG
#if ELPP_SYSTEM_LOG
#   define CSYSTEM_IF(writer, condition_, dispatchAction, ...)\
        ELPP_WRITE_LOG_IF(writer, (condition_), el::Level::System, dispatchAction, __VA_ARGS__)
#else
#   define CTRACE_IF(writer, condition_, dispatchAction, ...) el::base::NullWriter()
#endif  // ELPP_SYSTEM_LOG
#if ELPP_VERBOSE_LOG
...
#endif  // ELPP_TRACE_LOG
#if ELPP_SYSTEM_LOG
#   define CSYSTEM_EVERY_N(writer, occasion, dispatchAction, ...)\
        ELPP_WRITE_LOG_EVERY_N(writer, occasion, el::Level::System, dispatchAction, __VA_ARGS__)
#else
#   define CTRACE_EVERY_N(writer, occasion, dispatchAction, ...) el::base::NullWriter()
#endif  // ELPP_TRACE_LOG
#if ELPP_VERBOSE_LOG
...
#endif  // ELPP_TRACE_LOG
#if ELPP_SYSTEM_LOG
#   define CSYSTEM_AFTER_N(writer, n, dispatchAction, ...)\
        ELPP_WRITE_LOG_AFTER_N(writer, n, el::Level::System, dispatchAction, __VA_ARGS__)
#else
#   define CTRACE_AFTER_N(writer, n, dispatchAction, ...) el::base::NullWriter()
#endif  // ELPP_SYSTEM_LOG
#if ELPP_VERBOSE_LOG
...
#endif  // ELPP_TRACE_LOG
#if ELPP_SYSTEM_LOG
#   define CSYSTEM_N_TIMES(writer, n, dispatchAction, ...)\
        ELPP_WRITE_LOG_N_TIMES(writer, n, el::Level::System, dispatchAction, __VA_ARGS__)
#else
#   define CTRACE_N_TIMES(writer, n, dispatchAction, ...) el::base::NullWriter()
#endif  // ELPP_SYSTEM_LOG
#if ELPP_VERBOSE_LOG
